...
def generate_random_many_float(many,m_min,m_max,repeat,sort):
    global inputlist
    random_lst = []
    if repeat:
        for i in range(int(many)):
            random_lst.append(str(round(random.uniform(m_min,m_max),1)))
    else:
        for i in range(int(many)):
            random_lst.append(str(round(random.uniform(m_min,m_max),1)))
    if sort:
        random_lst.sort()
    inputlist.append(' '.join(random_lst)+'\n')
    return random_lst
...
for i in range(secret_count):
    ...
    for j in range(a):
        ...
        generate_random_many(b,1,b+1,0,0)
        generate_random_many_float(b,-40,40,0,0)
